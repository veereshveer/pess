<?php
	...
	if(isset($_POST['generate'])){
		...
		$query="select * from int_details where email = '$_POST[email]'";
		$exec=mysqli_query($con,$query) or die("Error");
		$count=@mysqli_num_rows($exec);
		if($count == 1){
		    
		    $q="select * from int_reg where email = '$_POST[email]'";
		    $qur=mysqli_query($con,$q) or die("Error");
		    $chk=mysqli_num_rows($qur);
		    if($chk == 0){
		$_SESSION['code']=rand(1000000,1111111);
		$to=$_POST['email'];
		$subject = 'Conformation Code';
		$messege = $_SESSION['code'];
		$headers = 'From: noreply@dr.ait';
			...
			<form method="post">
				<div>
					...
					<div class="input-group" style="padding:7px;">
						<span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
						<?php
						echo "<input class='form-control' placeholder='Conformation Code' type='password' name='ccode' required /><br/>";
						?>
					</div>
					<br/>
...
<?php
		...
		echo "<script>window.open('signup.php','_self');</script>";
		    ...
		    if($_SESSION['code'] == $_POST['ccode']){
		require_once('../db/db.php');
		$uname=$_SESSION['uname'];
		$pwd=$_SESSION['pwd'];
		$email=$_SESSION['email'];
		//require_once('../Functions/sendOTP.php');
		$exec = mysqli_query($con,"insert into int_reg values('$email','$uname','$pwd')");
		if($exec){
