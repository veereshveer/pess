...
if(isset($_POST['upload'])){
	$query = "update std_details set internal_marks='$_POST[marks]' where usn='$_POST[usn]'";
...
<body style="background-image:url(../images/643482.jpg);background-attachment:fixed;background-position:center;background-size:cover;">
	...
	<table class="table table-striped table-hover table-bordered" style="margin:10px;border-color:orange;" border="2" width="600px" height="40" style="border-color:white;">
			<tr>
			    <th>Guide</th>
			    <th>Group Name</th>
				<th>USN</th>
				...
				<th></th>
				<?php
				$q0="select * from int_details where email='$_SESSION[int_email]'";
				$query0 = mysqli_query($con,$q0) or die("Not working"); 
				$rows=mysqli_num_rows($query0);
				$num0 = mysqli_fetch_row($query0);
				
				$q="select * from std_details where course ='$num0[2]'";
				$query = mysqli_query($con,$q) or die("Not working"); 
				$num = mysqli_num_rows($query);
		while($num){
		$data=mysqli_fetch_row($query);
		$query1="select internal_marks from std_details where usn='$data[2]'";
