		...
		<div class="container text-center " style="margin-top:115px;width:530px;background-color:white;padding:30px;">
			<div>
				...
				<form method="post">
					...
					<input class="form-control" type="password" placeholder="Password" name="pwd" pattern=".{1,}" title="Minimum 7 digit" required/>
					</div>
					<br/>
...
<?php
	if(isset($_POST['login'])){
		$email = $_POST['email'];
		$pwd = $_POST['pwd'];
		$query=mysqli_query($con,"select * from adm_reg where email ='$email' and password = '$pwd'") or die("Not workin in qry");
		
		if(mysqli_num_rows($query)){
			$_SESSION['adm_email']=$_POST['email'];
